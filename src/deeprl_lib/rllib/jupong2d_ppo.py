...
def process_environment(agent, index, scale_factor, results_path, scale_factor_arr, play_steps,
...
def get_latest_checkpoint(results_path):
    """
    ...
    creation_dates = [creation_date(folder_path) for folder_path in list_subfolders_with_paths]
    max_ind = creation_dates.index(max(creation_dates))
    newest_folder = list_subfolders_with_paths[max_ind]
    checkpoint_number = get_latest_checkpoint_number(newest_folder)
...
def train_model(run, config, stop, output_folder, checkpoint_freq, save_folder, latest_checkpoint_path, as_test, stop_reward):
    """
...
def create_result_paths(output_folder, session, paddle_length):
    """
...
def rllib_configurations(run, env_name, num_workers, env_per_workers, stop_reward=None, stop_iters=None, stop_timesteps=None):
    """
...
def test_agent(env_name, config, latest_checkpoint_path, results_path, play_steps, paddle_length):
    """
    ...
    for i, sf in enumerate(scale_factor_arr):
        t = Thread(target=process_environment, args=(agent, i, sf, results_path, scale_factor_arr, play_steps, paddle_length))
        threads.append(t)
        t.start()
    for t in threads:
        t.join()
...
def start():
    ...
    # Creating the Gym-Environment JuPong2D
    register_gym_env(args.env_name, args.pl)
    print(f"Registered Gym-Environment: {args.env_name}")
    
    # Creating the configuration for the training or the testing
    ray.init(num_cpus=args.num_cpus or None)
    config, stop = rllib_configurations(args.run, args.env_name, args.num_workers, args.env_per_workers, stop_reward=args.stop_reward, stop_iters=args.stop_iters, stop_timesteps=args.stop_timesteps)

    # Result Path
    output_folder = args.output
    save_folder, results_path = create_result_paths(output_folder, args.session, args.pl)
    
    # Restore the latest checkpoint for training or testing
    latest_checkpoint_path = None
    if os.path.exists(results_path) and len(os.listdir(results_path)) and (args.restore or args.play):
        latest_checkpoint_path, checkpoint_number = get_latest_checkpoint(results_path)

    if not args.play:
        # Training section, where the tune command of rllib is used
        # More info about the tune command: https://github.com/ray-project/ray/blob/master/python/ray/tune/tune.py
        train_model(args.run, config, stop, output_folder, args.checkpoint_freq, save_folder, latest_checkpoint_path, args.as_test, args.stop_reward)
    else:
        # Section for analyzing parameters by playing with a trained agent
        test_agent(args.env_name, config, latest_checkpoint_path, results_path, args.play_steps, args.pl)
